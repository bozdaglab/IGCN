...
xtimes1 = 10
xtimes2 = 10
learning_rates = [0.01,0.005,0.001] # learning rates to tune IGCN
hd_sizes = [64,128,256,512] # hidden sizes to tune IGCN
#  run
print('setting up!')
import statistics
...
for i in range(len(base)):
    with open(base_path + 'dataset/' + netw_base[i] + '.csv') as f:
        df_ = pd.read_csv(f, header=None)
    df = torch.tensor(df_.values,device=device,dtype=torch.float32)
    DF[i] = df
